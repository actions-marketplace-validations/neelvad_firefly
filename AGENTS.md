...
- ✅ **Per-head attention attribution** — `capture --per-head` adds `layer.{i}.attn_heads` taps that capture the *input* to the attention output projection (o_proj/out_proj/dense/c_proj); `check` auto-detects these from `manifest.head_counts` and drills divergence down to the worst attention head per layer (`src/firefly/head_attribution.py`). Reports worst-head + concentration (worst/median head) in human/markdown/JSON. Genuine per-head only works on the o_proj *input* — post-projection heads are linearly mixed and unrecoverable. vLLM path: `scripts/capture_vllm.py --per-head` + `scripts/compare_per_head.py`. **GPU findings:** FLASH vs XFORMERS — exactly ONE head cracks at layer 7 on BOTH SmolLM (head 2/9) and Llama-3.1-8B (head 30/32), pattern universal across 60× scale; FLASH vs FLASHINFER (same image) — ALL heads diverge from layer 0, kernel-wide, no localization.
...
| `src/firefly/quant_risk.py` | Pure quant-risk core: `simulate_quant_error` (channel-mean rel err, per-tensor vs per-channel scales) / `tap_quant_risk` / `analyze_quant_risk`. `TapQuantRisk` with channel_concentration + mitigation_gain |
| `scripts/compare_per_head.py` | Diff two `--per-head` artifacts (e.g. two vLLM backends) and print the per-head attribution table |
